...
s=s.replace("  researchedRealProspectCount: 150,\n  researchedClassCounts: Object.freeze({ 2027: 87, 2028: 28, 2029: 3, 2030: 32 }),",
            "  minimumRealProspectCount: 150,\n  researchedClassCounts: Object.freeze({ 2027: 87, 2028: 28, 2029: 3, 2030: 48 }),")

anchor="  ['Cash Cieslak', 'F', 'Chicago Mission 14U AAA', '14U AAA', '2012-10-15', 'USA', null, null, 'R', 2030, 32],\n"
if anchor not in s:
...
new_rows="""  ['RJ Celebrini', 'F', 'North Shore WC U15 A1', 'U15 A1', '2012-08-18', 'Canada', '5\\'3\"', 115, 'L', 2030, 33],
  ...
  [\"Brooks Brind'Amour\", 'F', 'Carolina Jr. Hurricanes 14U AAA', 'T1EHL 14U', '2011-12-19', 'USA', null, null, null, 2030, 35],
  ['Jack Allgood', 'F', 'MN Lakers 13U AAA', 'MNHP 13O', '2012-06-11', 'USA', null, null, 'L', 2030, 36],
  ...
  ['Marcellus McField', 'D', 'Burnaby Winter Club U15 AAA', 'CSSHL U15 AAA', '2011-12-24', 'USA', null, null, 'L', 2030, 45],
...
for line in new_rows.splitlines():
    name=line.split("['",1)[1].split("'",1)[0] if "['" in line else None
    if name and name in s:
        raise SystemExit(f'duplicate candidate already present: {name}')
s=s.replace(anchor,anchor+new_rows,1)

old="""if (REAL_PROSPECTS.length !== PROJECT_ICE_PROSPECT_RULES.researchedRealProspectCount) {
  throw new Error(`[Project Ice] Expected 150 real prospects, found ${REAL_PROSPECTS.length}.`);
}
"""
new="""if (REAL_PROSPECTS.length < PROJECT_ICE_PROSPECT_RULES.minimumRealProspectCount) {
  throw new Error(
