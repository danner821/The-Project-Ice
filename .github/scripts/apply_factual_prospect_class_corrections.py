...
prospect_path = Path('artifacts/project-ice/public/prospects.js')
world_path = Path('artifacts/project-ice/public/world.js')
ps = prospect_path.read_text(errors='ignore')
ws = world_path.read_text(errors='ignore')

old_counts = "researchedClassCounts: Object.freeze({ 2027: 87, 2028: 28, 2029: 3, 2030: 48 }),"
new_counts = "researchedClassCounts: Object.freeze({ 2027: 87, 2028: 28, 2029: 29, 2030: 47 }),"
if old_counts not in ps:
    raise SystemExit('class counts anchor missing')
ps = ps.replace(old_counts, new_counts, 1)

old_2029 = """  ['Madden Daneault', 'F', 'Red Deer Rebels U15 AAA', 'AEHL U15', '2011-05-07', 'Canada', '5\\'11"', 172, 'R', 2029, 1],
...
new_2029 = """  ['Madden Daneault', 'F', 'Red Deer Rebels U15 AAA', 'AEHL U15', '2011-05-07', 'Canada', '5\\'11"', 172, 'R', 2029, 1],
...
if old_filter not in ws:
    raise SystemExit('external prospect source-prune anchor missing')
ws = ws.replace(old_filter, new_filter, 1)

prospect_path.write_text(ps, encoding='utf-8')
world_path.write_text(ws, encoding='utf-8')
print('FACTUAL_PROSPECT_CLASSES=2027:87,2028:28,2029:29,2030:47,TOTAL:191')
