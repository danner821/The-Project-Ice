...
# 1) Add the dedicated Film Study runtime. Recovery remains a compatibility
# backend event type so old saves still complete through the canonical event
# result pipeline, but the player-facing mechanic is now Film Study.
film_study = r'''\
'use strict';
...
(() => {
  ...
  const choicesForPlayer = player => {
      ...
      return [
        {
          ...
          rewards: { puckTracking: 4, anticipation: 3, positioning: 2 },
          category: 'goalie',
        },
        {
          key: 'movement',
          ...
          rewards: { lateralMovement: 4, recoverySpeed: 3, angles: 2 },
          category: 'goalie',
        },
        {
          key: 'rebound',
          ...
          rewards: { reboundControl: 4, stickControl: 3, composure: 2 },
          category: 'goalie',
        },
      ];
    }
    ...
    return [
      ...
      },
      {
        ...
        rewards: { offensiveAwareness: 3, defensiveAwareness: 3, passing: 3 },
        category: 'hockeyIQ',
      },
    ];
  };
  ...
  function normalizeSchedule(options = {}) {
    ...
    for (const event of schedule) {
      if (normalizeFilmStudyEvent(event)) changed = true;
    }
  ...
  function resolveCurrentEvent() {
    ...
    if (ids.length) {
      const exact = schedule.find(event =>
        [event?.eventId, event?.id, event?.canonicalEventId]
          .filter(Boolean)
  ...
  function applyFocusXP(player, choice, event) {
    ...
    dev.xpEarnedByCategory[choice.category] =
      Math.max(0, Number(dev.xpEarnedByCategory[choice.category]) || 0) + total;
  ...
  function injectStyles() {
    ...
    style.textContent = `
      #pi-film-study-screen { position: fixed; inset: 0; z-index: 10050; overflow-y: auto; background: radial-gradient(circle at 50% -10%, rgba(45,86,165,.24), transparent 42%), #060b16; color: #f4f7ff; padding: max(22px, env(safe-area-inset-top)) 18px max(34px, env(safe-area-inset-bottom)); }
      .pi-film-study__shell { width: min(100%, 520px); margin: 0 auto; }
      .pi-film-study__back { appearance:none; border:0; background:transparent; color:#9fb0ce; font:inherit; padding:8px 0 18px; }
      .pi-film-study__hero { border:1px solid rgba(108,146,214,.22); background:linear-gradient(180deg, rgba(18,31,57,.94), rgba(10,18,34,.94)); border-radius:22px; padding:22px; box-shadow:0 18px 55px rgba(0,0,0,.28); }
      .pi-film-study__eyebrow { margin:0 0 8px; color:#779be0; font-size:12px; font-weight:800; letter-spacing:.12em; text-transform:uppercase; }
      .pi-film-study__title { margin:0; font-size:28px; line-height:1.05; }
      .pi-film-study__copy { margin:12px 0 0; color:#aebbd1; line-height:1.55; font-size:14px; }
      .pi-film-study__choices { display:grid; gap:12px; margin-top:16px; }
      .pi-film-study__choice { width:100%; text-align:left; color:inherit; border:1px solid rgba(119,155,224,.18); background:rgba(15,26,47,.82); border-radius:18px; padding:16px; display:grid; grid-template-columns:40px 1fr; gap:12px; align-items:start; }
      .pi-film-study__choice:active { transform:scale(.99); background:rgba(24,42,75,.9); }
      .pi-film-study__icon { width:40px; height:40px; display:grid; place-items:center; border-radius:12px; background:rgba(76,119,201,.13); font-size:20px; }
      .pi-film-study__choice strong { display:block; font-size:16px; margin-bottom:5px; }
      .pi-film-study__choice span { display:block; color:#9eacc3; font-size:13px; line-height:1.4; }
      .pi-film-study__reward { margin-top:8px; color:#79a7ff !important; font-size:12px !important; font-weight:700; }
    `;
  ...
  function openFilmStudy() {
    ...
    root.innerHTML = `
      <div class="pi-film-study__shell">
        ...
        <div class="pi-film-study__choices">
          ${choices.map(choice => `
            <button type="button" class="pi-film-study__choice" data-film-choice="${choice.key}">
              <span class="pi-film-study__icon">${choice.icon}</span>
              <span>
                <strong>${choice.title}</strong>
                <span>${choice.description}</span>
                <span class="pi-film-study__reward">Small focused attribute XP</span>
              </span>
            </button>
          `).join('')}
        </div>
    ...
    root.querySelectorAll('[data-film-choice]').forEach(button => {
      button.addEventListener('click', async () => {
        ...
        const base = WorldEngine.completeRecoveryEvent?.(eventId, { save: false }) || {
          success: true,
          applied: false,
        };
        ...
        const result = {
          ...
          focusLabel: choice.title,
          xp: {
            ...((base?.result?.xp || base?.xp) || {}),
            attributes: { ...choice.rewards },
            general: totalXP,
          },
  ...
  if (typeof COMPLETE_SCREENS !== 'undefined') {
    COMPLETE_SCREENS['film-study'] = openFilmStudy;
  }
...
'''
(PUBLIC / 'film-study-event.js').write_text(film_study, encoding='utf-8')

# 2) Make postseason cadence create Film Study events rather than Recovery.
cadence_path = PUBLIC / 'postseason-cadence.js'
cadence = cadence_path.read_text(encoding='utf-8')
cadence = cadence.replace('const MODULE_VERSION = 3;', 'const MODULE_VERSION = 4;')
cadence = cadence.replace("if (previous && addDays(previous, 1) === date) return 'recovery';", "if (previous && addDays(previous, 1) === date) return 'recovery';")
cadence = cadence.replace("event.eventKey = practice ? 'practice-systems' : 'recovery';", "event.eventKey = practice ? 'practice-systems' : 'film-study';")
cadence = cadence.replace("event.label = practice ? 'Playoff Practice' : 'Recovery Session';", "event.label = practice ? 'Playoff Practice' : 'Playoff Film Study';")
cadence = cadence.replace("event.shortLabel = practice ? 'Practice' : 'Recovery';", "event.shortLabel = practice ? 'Practice' : 'Film Study';")
cadence = cadence.replace("event.icon = practice ? '🏒' : '😴';", "event.icon = practice ? '🏒' : '🎥';")
cadence = cadence.replace("event.location = practice ? 'Team Rink' : 'Training Facility';", "event.location = practice ? 'Team Rink' : 'Video Room';")
cadence = cadence.replace(": 'Reset physically and mentally between postseason games.';", ": 'Review the last game and sharpen one detail before the next postseason test.';")
cadence = cadence.replace(": 'A postseason recovery day focused on resetting before the next test.';", ": 'A focused postseason video session built around the last game and the next opponent.';")
cadence = cadence.replace("event.focus = practice ? 'systems' : null;", "event.focus = practice ? 'systems' : 'film-study';\n    if (!practice) event.completeScreen = 'film-study';")
cadence = cadence.replace("event.requiresPlayerInteraction = practice;", "event.requiresPlayerInteraction = true;")
cadence_path.write_text(cadence, encoding='utf-8')

# 3) Inject the focused runtime after the calendar/lifecycle systems are loaded.
vite_path = ROOT / 'vite.config.ts'
vite = vite_path.read_text(encoding='utf-8')
anchor = "    if (!html.includes('/career-calendar-projection.js')) scripts.push('    <script src=\"/career-calendar-projection.js\"></script>');\n"
line = "    if (!html.includes('/film-study-event.js')) scripts.push('    <script src=\"/film-study-event.js\"></script>');\n"
if line not in vite:
    if anchor not in vite:
        raise SystemExit('vite injection anchor not found')
    vite = vite.replace(anchor, anchor + line)
vite_path.write_text(vite, encoding='utf-8')

print('Replaced Recovery presentation with interactive Film Study and loaded the runtime.')
