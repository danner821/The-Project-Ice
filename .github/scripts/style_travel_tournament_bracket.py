...
if 'function travelTeamAbbr(team)' not in text:
    start = text.find('\n  function bracket(state) {')
    end = text.find('\n  function leaders(state) {', start)
    if start < 0 or end < 0:
        raise SystemExit(f'Could not locate bracket block: start={start}, end={end}')
