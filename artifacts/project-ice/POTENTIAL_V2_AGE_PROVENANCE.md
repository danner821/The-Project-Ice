...
## Verified generation-stamp provenance and first-week evaluator guard

- A second **direct full private Backup 3 inspection** established stronger evidence than the age-only heuristic: **all 53** historically inconsistent current freshmen carry `generatedIncomingFreshman: true` and an `incomingClassSeasonId`. **Every one of those 53 has one or more archived seasons that predate the player's recorded incoming class.** The other **95** generated players have no such pre-generation archive flag. Cohorts: 13 incoming in 2024 with a preceding 2023 archive; 40 incoming in 2025 with a preceding 2024 archive (most also 2023). This is direct archived provenance conflict, not merely implausible ages.
- Read-only `tools/potential-v2-validated-review.js` now checks that incoming-class stamp **before** accepting plausible ages. Even if a recycled archived age happens to match the new freshman's age, any archived season preceding their incoming year triggers `ARCHIVE_PREDATES_PLAYER_GENERATION` and quarantines their entire historical evaluation and peer contribution. Genuine new freshmen whose completed histories start no earlier than their incoming class remain eligible. The updated test exercises both cases and input immutability.
- Re-ran this updated guard directly on both original private revision-4 and Backup 3 revision-16 saves: **95 coherent / 53 generation-stamp quarantined**, identical corrected shadow, calibration and evidence aggregates on both files. All **95** previously accepted player-level review objects remained byte-for-byte identical; this upgrade adds stronger *why* evidence for the same 53, not new ratings.
- Extended the *offline-only* V2 weekly shadow evaluator with the same pre-generation identity gate for **both subjects and peer pools**; it also excludes real/curated players and root-versus-development potential-conflicted peers pending separate review. Unit and long-run simulation suites PASS. The legacy production weekly evaluator is unchanged; this shadow is **not** imported by live gameplay.
- Applied the current week's offline V2 entry gate to all **160** real saved high-school roster players in Backup 3 at the current `2025-09-04` preseason state: **53 withheld for inherited archived identity**, **12 real players withheld pending future separate rules**, **1 career player withheld for unresolved root 74/development 68 potential conflict**, and **94 withheld because no current-season games exist yet**. **Zero immediate numerical potential changes** can legitimately occur at this baseline; earlier two-season breakout labels remain a *historical review signal*, not a current-week promotion.
- The original private backup and Backup 3 SHA-256 digests were verified unchanged. Prior future-only freshman generation patch remains in effect. The next proper work is **explicitly reviewed numeric seeding for the one conflicted career player, and provenance-safe prospective weekly integration**, never automated rerating of inherited-history players. Keep current live career and private rollback untouched until approval.


## Hard migration block and both saved-potential clone scenarios

- The offline clone-only migration rehearsal now **rejects any explicit review plan targeting an incoming freshman with an archive predating their generated class**, even if the plan sets `reviewed:true`. This fail-closed check reuses the weekly identity guard, so source-scoring and target-migration eligibility cannot disagree. Synthetic rejection, normal canonical migration, and external-real-prospect prohibition checks PASS.
- Re-ran the **strict exhaustive 56 MB Backup 3 clone audit** under the expanded guard for both of Alacai's **existing unresolved** ratings and one *arbitrary stress-test* rating: **68 (16 authorized potential-related changed leaves), 74 (14), 79 (17)**. The 79 case is purely a robustness fixture, **not an evidence-based recommendation**. Every diff matched the strict potential-field/root-mirror/history allowlist. All roster players other than the one deliberately selected career player, all XP, stats, attributes, season and schedule fields, and all 191 external prospects remained identical to Backup 3. Source backup SHA-256 remained unchanged.
- These checks are *offline clone simulations only*. None of the three numbers is an approved new Potential 2.0 seed. The official canonical-seed decision still requires a separately reviewed justification for the current root **74** versus nested **68** and explicit user approval. Do not migrate the 53 provenance-quarantined players or silently assume their inherited XP or prospects are clean.
