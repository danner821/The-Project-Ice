...
(() => {
  ...
  function prospectRank(player){
    ...
    const ids=new Set([String(player.sourcePlayerId||''),String(player.playerId||''),String(player.id||'')].filter(Boolean)),rows=rankRows();
    for(let i=0;i<rows.length;i++){
      const row=rows[i],rid=String(row?.playerId||row?.id||row?.prospectId||'');
      if(rid&&ids.has(rid))return Number(row?.rank??row?.prospectRank??row?.ranking??i+1);
    }
    if(player.isRealProspect===true){
      const n=nameOf(player).toLowerCase(),i=rows.findIndex(r=>nameOf(r).toLowerCase()===n);
      if(i>=0)return Number(rows[i]?.rank??rows[i]?.prospectRank??rows[i]?.ranking??i+1);
    }
    return null;
  }
  function decorate(root,team){
    if(!root||!team)return;
    root.querySelectorAll('.pi-prospect-rank-badge').forEach(n=>n.remove());
    for(const card of root.querySelectorAll('[data-player-id]')){
      ...
      if(!rank)continue;
      const target=card.querySelector('.lineup-player__overall')||card.querySelector('.lineup-player__name')||card.querySelector('[class*="overall"]')||card.querySelector('[class*="name"]')||card;
      const b=document.createElement('small');
      b.className='pi-prospect-rank-badge';
      b.textContent=`Prospect #${rank}`;
  ...
  /*
   * Prospect rank badges have one native owner: renderTeamTab.
   * Team Profiles are built by rendering this same Team tab and cloning it,
   * so decorating immediately after the canonical Team render automatically
   * covers My Team, HS Team Profiles and Travel Team Profiles without a DOM
   * observer or a second custom lineup renderer.
   */
  function installProspectBadgeOwnership(){
    if(typeof globalThis.renderTeamTab!=='function'||globalThis.__projectIceProspectBadgesWrapped)return false;
    const original=globalThis.renderTeamTab;
    globalThis.__projectIceProspectBadgesWrapped=true;
    globalThis.renderTeamTab=function(teamId=null){
      const out=original(teamId);
      const selectedId=teamId||Game?.teamTabSelectedTeamId||realCareerTeamId();
      const team=(WorldEngine.state?.teams||[]).find(t=>String(t?.teamId||'')===String(selectedId||''))||null;
      decorate(document.getElementById('team-page-root'),team);
      return out;
    };
    return true;
  }
  ...
  function install(){injectStyle();installTeamTabOwnership();installProspectBadgeOwnership();if(typeof WorldEngine.openTravelTeamProfile!=='function'||WorldEngine.__travelProfileRepairV4Wrapped)return false;const original=WorldEngine.openTravelTeamProfile.bind(WorldEngine);WorldEngine.__travelProfileRepairV4Wrapped=true;WorldEngine.openTravelTeamProfile=function(teamId){const state=WorldEngine.rebuildTravelHockeyRosters?.()||travel();activeTravelTeam=(state?.teams||[]).find(t=>String(t.teamId)===String(teamId))||null;restoreTeamId=realCareerTeamId();const out=original(teamId);requestAnimationFrame(()=>requestAnimationFrame(()=>{patch(activeTravelTeam);restoreHsTeam();}));return out;};return true;}
