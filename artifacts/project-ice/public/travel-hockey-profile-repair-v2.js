...
(() => {
  ...
  function restoreHsTeam(){
    ...
    try{
      ...
      /*
       * renderTeamProfile builds a Travel profile by temporarily rendering the
       * adapter through the shared Team-tab renderer, then cloning that markup.
       * Removing the adapter and restoring only the selected ID leaves the
       * underlying Team-tab DOM showing the last Travel club. Re-render the
       * canonical HS team immediately after the profile clone is complete so
       * the persistent Team tab never inherits temporary Travel presentation.
       */
