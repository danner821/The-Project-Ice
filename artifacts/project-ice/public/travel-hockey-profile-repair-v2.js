...
(() => {
  ...
  function install(){injectStyle();installTeamTabOwnership();if(typeof WorldEngine.openTravelTeamProfile!=='function'||WorldEngine.__travelProfileRepairV4Wrapped)return false;const original=WorldEngine.openTravelTeamProfile.bind(WorldEngine);WorldEngine.__travelProfileRepairV4Wrapped=true;WorldEngine.openTravelTeamProfile=function(teamId){const state=WorldEngine.rebuildTravelHockeyRosters?.()||travel();activeTravelTeam=(state?.teams||[]).find(t=>String(t.teamId)===String(teamId))||null;restoreTeamId=realCareerTeamId();const out=original(teamId);requestAnimationFrame(()=>requestAnimationFrame(()=>{patch(activeTravelTeam);restoreHsTeam();}));return out;};return true;}
