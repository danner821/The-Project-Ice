...
(() => {
  ...
  function rebuild(state){
    ...
    const mine=state.teams.find(t=>String(t.teamId)===String(state.playerTeamId))||state.teams[0];if(mine){mine.roster=mine.roster.filter(p=>!p.isCareerPlayer&&pid(p)!==pid(career));const careerBucket=pos(career);const replace=mine.roster.findIndex(p=>pos(p)===careerBucket);if(replace>=0)mine.roster.splice(replace,1);else if(mine.roster.length)mine.roster.pop();career.position=naturalPos(career);if(career.position==='D')career.position='LD';career.travelStats=oldStats.get(pid(career))||career.travelStats||blankStats();mine.roster.unshift(career);mine.roster=mine.roster.slice(0,20);lineup(mine);}
