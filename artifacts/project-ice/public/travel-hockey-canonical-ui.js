...
(() => {
  ...
  function leaders(state) {
    ...
    return rows.map((item,index) => `<div class="pi-ts-leader" data-player="${esc(item.player.playerId || item.player.sourcePlayerId || '')}" data-team="${esc(item.team.teamId)}"><span>${index + 1}</span><span class="pi-ts-leader__identity"><span>${esc(item.player.name)}</span><small>${esc(item.team.shortName || item.team.name)}</small></span><span class="v">${Number(item.player.travelStats?.pts || 0)} PTS</span></div>`).join('');
  }
